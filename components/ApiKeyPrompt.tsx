...
const ApiKeyPrompt: React.FC<ApiKeyPromptProps> = ({ onKeySubmit, error }) => {
    ...
    const handleSubmit = async (e: React.FormEvent) => {
        ...
        // The parent component's state change will handle unmounting this component
        // or re-rendering it with an error. We just need to handle the local loading state.
        setIsVerifying(false);
    };

    return (
        <div className="fixed inset-0 bg-gray-900 z-50 flex items-center justify-center p-4">
            <div className="w-full max-w-md m-auto bg-gray-800 border border-cyan-500/30 rounded-2xl shadow-2xl text-white p-8">
                ...
                <form onSubmit={handleSubmit}>
                    ...
                    />
                    {error && (
                        <div className="text-red-400 text-sm mt-4 p-3 bg-red-500/10 rounded-lg whitespace-pre-wrap text-left">
                            {error}
                        </div>
                    )}
                    <button
