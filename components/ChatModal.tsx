...
const ChatModal: React.FC<ChatModalProps> = ({
  ...
  useEffect(() => {
    if (messages.length < 3) { // Welcome message
  ...
  return (
    <div className="fixed inset-0 bg-black/60 z-50 flex items-center justify-center p-0 md:p-4 backdrop-blur-sm animate-fade-in">
      <div
        className="bg-gray-900 md:border border-cyan-500/30 md:rounded-2xl shadow-2xl text-white w-full h-full md:max-w-4xl md:h-[90vh] flex flex-col transform transition-all md:animate-fade-in-up"
        onClick={(e) => e.stopPropagation()}
      >
        <header className="p-4 border-b border-gray-700/50 flex items-center justify-between flex-shrink-0">
          <h2 className="text-xl font-bold">Chat de Agentes IA</h2>
          <button
            onClick={onClose}
        ...
        <div className="flex-1 grid grid-cols-1 md:grid-cols-3 min-h-0">
            <div className="md:col-span-2 flex flex-col md:border-r border-gray-700/50">
                 <header className="p-4 border-b border-gray-700/50 flex items-center flex-shrink-0">
